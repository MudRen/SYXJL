...
int do_copy(object me, object ob)
{
        ...
        me->set("title", HIC"第" + chinese_number(me->query("generation")) + "代赏善使者"NOR);
	me->set("short", me->query("title") + " " + me->query("name") + "(Shangshan shizhe " + capitalize(ob->query("id")) + ")");
        ob->set("title",HIC "第" + chinese_number(me->query("generation")) + "代赏善使者" NOR);
/*
        me->set("title", HIC"第" + chinese_number(me->query("generation")) + "代赏善使者"NOR);
