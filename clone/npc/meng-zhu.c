...
int do_kill()
{
        object ob;
        int i;
        command("say 你想谋害本盟主，当真是自找死路！ 座下白衣武士何在！");
...
int checking(object me, object ob)
{
        ...
        if (( (int)me->query("qi")*100 / my_max_qi) <= 40 ) {
                command("say 佩服佩服，看来我的担子可以交卸了，恭喜你成为武林盟主！\n");
...
int do_copy(object me, object ob)
{
        ...
        me->set("title", HIC"第" + chinese_number(me->query("generation")) + "代武林盟主"NOR);
	me->set("short", me->query("title") + " " + me->query("name") + "(Mengzhu " + capitalize(ob->query("id")) + ")");
...
int do_clone(object me, object ob)
{
        ...
        int i, temp; /* delete and copy skills */
...
/* wield and wear weapon & armor */
        ...
        for(i=0; i<sizeof(inv); i++) {
                if( inv[i]->query("weapon_prop/damage") > 30
                ||  inv[i]->query("zizhi")
                ||  inv[i]->query("armor_prop/armor") > 50 ) continue;
                if( inv[i]->query("weapon_prop") &&  inv[i]->query("equipped") ) {
			this_object()->carry_object(base_name(inv[i]))->wield();
                        me->set("weapon", base_name(inv[i]));
                }
                else if( inv[i]->query("armor_prop") &&  inv[i]->query("equipped") ) {
			this_object()->carry_object(base_name(inv[i]))->wear();
