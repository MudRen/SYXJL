...
{       
        if(!(who->query_temp("club") ) ){
                message_vision(HIC"师傅没有同意为你铸棍，您请回吧！\n"NOR, who);
        ...
        message("vision","侍棍低头检视了一下手中的"+(string)ob->query("name")+
                        "点了点头说：\n", who);
        message("vision","。。。好我们现在就开始铸棍\n", who);

        write("请这位"+RANK_D->query_respect(who)+"给棍起名字吧！(zhu 棍的名字 英文代号)\n");
                remove_call_out("destrory");
                call_out("destrory",1,ob);
        return 1;
...
int do_zhu(string arg)
{
        ...
        if(!w_name||!id)
                return notify_fail("什么?(使用zhu 棍的名字 英文代号\n)");
        i = strlen(id);
        while(i--)
                if( id[i]<'a' || id[i]>'z' )
                        return notify_fail("对不起，棍的英文代号只能用英文字母。\n");
        ...
        message_vision(me->name()+"沉吟了一会，对侍棍巧声说了几句话。侍棍点了点头。说：好吧！\n",me );
        ...
        club=new("/d/npc/m_weapon/weapon/m_club",1);    //生成新棍
        club->move(this_player());
        message_vision("侍棍摸起地上，带着斑斑血迹还有些烫手的巨棍，说：\n棍。。已。。成。。，侍棍的任务。。。也就完成了。。。。\n",me );
        say("侍棍艰难的说：棍。。您。。收好，我该走了。。。\n");
        message_vision("侍棍说完。倏的便不见了。\n",me);
