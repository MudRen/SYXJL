// Room: /d/jingzhou/npc/xunbu.c
// Date: 99/06/01 by Byt

#include <ansi.h>
#define THIEF 3
#define ROBBER 5
#define GO 0
#define BACK 1

inherit NPC;

void patrol();
void catchyou(object);
int mygo(string);
//int do_go(string arg);
//int do_quit(string arg);
int accept_bribery(string);

int direction=GO;

mapping BigWay=([
                "/d/jingzhou/yamen":({"south",0}),
                "/d/jingzhou/jingzhonglu2":({"east","north"}),
                "/d/jingzhou/jingzhonglu":({"east","west"}),
                "/d/jingzhou/zhangjuzhengjie":({"north","west"}),
                "/d/jingzhou//d/jingzhou/bingyingdamen":({"north","south"}),
                "/d/jingzhou/quyuanlu2":({"north","south"}),
                "/d/jingzhou/jingnanlu":({"east","west"}),
                "/d/jingzhou/wanjiahouyuan":({"east","west"}),                
                "/d/jingzhou/wanjiahouyuan":({"east","west"}),                
                "/d/jingzhou/wanjiadazhai":({"east","west"}),                                
                "/d/jingzhou/nvrenjie1":({"east","west"}),
                "/d/jingzhou/nvrenjie3":({"east","west"}),
                "/d/jingzhou/huatai":({"east","west"}),                
                "/d/jingzhou/kedian":({"east",0}),
                "/d/jingzhou/jiulou": ({0,"west"}),
                "/d/jingzhou/qianzhuang": ({0,"south"}),
                "/d/jingzhou/yaopu": ({0,"east"}),
                ]);

void create()
{
     string *surname=({"ÕÔ","Ç®","Ëï","Àî","ÖÜ","Îâ","Ö£","Íõ"});
     set_name(surname[random(sizeof(surname))]+"²¶Í·", ({ "bu tou"}) );
     set("gender", "ÄĞĞÔ" );
     set("age", 25);
     set("str", 45);
     set("cor", 50);
     set("cps", 35);
     set("int", 20);
     set("per", 20);
     set("kar", 15);
     set("long",@LONG
²¶Í·¸ö¸öÉíÊÖ½Ã½¡£¬Ç¿µÁ¡¢É½ÔôÎŞ²»ÎÅ·çÉ¥µ¨¡£
LONG
);
     set("status","patrol");
     set("CatchWho",0);
     set("combat_exp", 500000);
     set("attitude", "heroism");
     set("chat_chance_combat", 40);
     set("chat_msg_combat", ({
                            "²¶Í·ËµµÀ£ºÄãÏë¸ÉÊ²Ã´? »¹²»¸úÎÒÈ¥ÏØÑÃÊÜÉó!?\n",
                            "²¶Í·ËµµÀ£º´óµ¨! ¾ÓÈ»¸Òµ±½Ö¾Ü²¶! ÏëÔì·´£¿\n",
                            "²¶Í·ÊÖÖĞµÄµ¶Ô½¿³Ô½¿ì£¬ÈÃÄãÓ¦½Ó²»Ï¾¡£\n",
                            "²¶Í·ËµµÀ£º±ğºúÄÖÁË£¬»¹ÊÇ¹Ô¹ÔµÄ¸úÎÒ×ß°É¡£\n"
                            }) );
     set ("inquiry",([
                "bribery":
"ËµÄÄÀï»°À´£¬ÏØÌ«Ò¯ÇåÆ¶Á®Õı£¬ÎÒÃÇ×÷ÊÖÏÂµÄÆñÄÜÈèÃ»ËûµÄÃûÉù£¿\n"+
"ÊÕÆğÄãµÄÇ®°É£¡\n",
                "salary":
"°¥£¬ÀÏÒ¯ÙºÒøÎ¢±¡£¬ÎÒÃÇµÄÄêâÃ×ÔÈ»¸üÉÙÁË......\n",
                "money":
"ËµÊµÔÚµÄ£¬ÎÒÒ²ÏëÒªÇ®¡£µ«ÒªÊÇÈÃÀÏÒ¯ÖªµÀÁËÎÒÊÕÊÜ»ßÂ¸(bribery)£¬\n²»ËÀÒ²µÃ°Ç²ãÆ¤\n
",
                        ]));
     set ("chat_chance",99);
     set ("chat_msg", ({
                      (:call_out,"patrol",0:),
                      }) );
     set("pursuer",1);
     set_skill("blade", 100);
     set_skill("parry", 90);
     set_skill("dodge", 70);
     setup();
     direction=GO;
     carry_object("/clone/weapon/gangdao")->wield();
     carry_object("/clone/misc/cloth")->wear();
     add_money("silver",30);
}

void init()
{
	object catchwho,you,*npc;
	object oldman,start_room;
        int i;
	catchwho=query("CatchWho");
	you=this_player();
	if (query("status")=="patrol")
   	{
   		if (!is_fighting()&&interactive(you)&&living(you))
      		{
        		npc=all_inventory(environment());
        		i=sizeof(npc);
        		while (i--) {
                		if (npc[i]->query("id")=="gangster"&&!userp(npc[i]))
                        	{
                        		say(HIW "½ñ¶ùÊÇÊ²Ã´ÈÕ×Ó£¿\n"+"¾ÓÈ»"+
                                	npc[i]->query("name")+
                               		"Ò²¸ÒÔÚ´ó½ÖÉÏÁïßÕ£¡" NOR);
                        		npc[i]->remove_all_enemy();
                        		say("È¥ËÀ°É£¡£¡\n");
                        		EMOTE_D->do_emote( npc[i],"fear");
                        		kill_ob(npc[i]);
                        		npc[i]->kill_ob(this_object());
                        	}
                }
        if (you->query("thief")&&random(10)<7)
           {
        	say (HIR "ÄÖÁË°ëÌìÔ­À´"+you->query("name")+
	                        "ÊÇ¸öÔô°¡! ÎÒ×ßÒ»ÌË°É!\n" NOR);
           	you->add("crime_lv",THIEF);
           	you->add("thief",-1);
           	if (!you->query("thief")) you->delete("thief");
           	catchyou(you);
                return;
           }
        else
            if (you->query("marks/Ç¿µÁ")&&random(10)<9)
               {
               message_vision("$N¶Ô×Å$nºÈµÀ£º¹âÌì»¯ÈÕÖ®ÏÂ£¬Äã¾ÓÈ»Ò²¸ÒĞĞĞ×É±ÈË£¿
»¹²»ÓëÎÒ»ØÈ¥³Ô°å×Ó£¡\n"
                +"Ëµ°Õ£¬$N½âÏÂÌúÁ´Ì×ÔÚ$nµÄ²±×ÓÉÏ¡£\n",this_object(),you);
                        you->add("crime_lv",ROBBER);
                        you->add("marks/Ç¿µÁ",-1);
                        if (!you->query("marks/Ç¿µÁ")) you->delete("marks/Ç¿µÁ");
                catchyou(you);
                }
        }
   }
	else if (query("status")=="catching")
   	{
   		catchyou(catchwho);
   		message_vision(HIR "Ö»¼û$NÍ»È»´ÓÑü¼ä½âÏÂÒ»ÌõÌúÁ´Ì×ÔÚ$nµÄ²±×ÓÉÏ¡£\n"NOR,
                this_object(),catchwho);
	}
	else {
      		if (objectp(catchwho=this_object()->query("CatchWho")))
        	{
            	if (BigWay[file_name(environment())][direction]==0)
                        {
        			say("try to patrol\n");
                        	start_room=find_object("/d/jingzhou/yamen.c");
                        	oldman=find_object("/d/jingzhou/npc/lingtuisi.c");
                        	oldman=present(oldman->query("id"),start_room);
                        	write(oldman->query("id"));
                        	catchwho->move(start_room);
                        	move(start_room);
                        	say("ÀÏÒ¯£¬ÈË·¸ÒÑ´øµ½£¬Ğ¡ÈËÑ²½ÖÈ¥ÁË¡£\n");
                        	set("CatchWho",0);
                        	direction=GO;
                        	set("status","patrol");
                        	move(find_object(query("startroom")+".c"));
                        	patrol();
                        	call_other(oldman,(:call_out,"settle_lawsuit",catchwho,2:));
                        	return;
                        }
                }
        }
//add_action("do_go","go");
//add_action("do_quit","quit");
add_action("accept_bribery","bribe");
}

void patrol()
{
int i,on_way;
        object *content,catchwho,start_room,crime;
	string *bigwayname,where,dir;
	bigwayname=keys(BigWay);
	where=file_name(environment());
	write("in patrol\n");
	for (i=0;i<sizeof(bigwayname);i++)
    	{
    		if (where==bigwayname[i])
       		{
	       		on_way=1;
       			break;
       		}
    		else on_way=0;
    	}
	if (on_way)
   	{
   		if (this_object()->query("status")=="patrol")
      		{
      			if (!BigWay[where][direction]) 
      				direction=1-direction;
      		}
   			else if (this_object()->query("status")=="catching")
      				{
      					catchyou(catchwho);
      					message_vision(HIR "Ö»¼û$NÍ»È»´ÓÑü¼ä½âÏÂÒ»ÌõÌúÁ´Ì×ÔÚ$nµÄ²±×ÓÉÏ"NOR,
                        		this_object(),catchwho);
      				}
   			     else
      				{
        				write("here???\n");
         			}
  	}
	else
   	{
   		content=all_inventory(this_object());
   		for (i=0;i<sizeof(content);i++)
       {
       if (content[i]->query("id")=="amulet")
                {
                if (!stringp(dir=content[i]->query("map")[where]))
                        {
                        move(start_room=find_object(query("startroom")+".c"));
                        if (objectp(crime=query("CatchWho")))
                                {
                                if (where==file_name(environment(crime)))
                                        crime->move(start_room);
                                }
                        }
                else mygo(dir);
          break;
          }
  }
return;
}
mygo(BigWay[where][direction]);
}

void catchyou(object crime)
{
	object ob;
	ob=this_object();
	while (environment(ob)!=environment(crime))
        	move(environment(crime));
		crime->set_leader(ob);
		set("CatchWho",crime);
		set("status","caught");
		crime->set("being_caught",1);
		direction=BACK;
		return;
}

int mygo(string dir)
{
//GO_CMD->main(this_object(),dir);
	command("go "+dir);
	return 1;
}

int do_go(string arg)
{
	object me;
	me=this_player();
	if (!me->query("being_caught")) return 0;
	tell_object(me, "ºÙºÙ, »¹ÏëÍùÄÄÌÓ?!\n");
	return 1;
}

int do_quit(string arg)
{
	object me;
	me=this_player();
	if (!me->query("being_caught")) return 0;
	if (is_fighting()&&random(20)<16)
        {
	        me->delete("being_caught");
        	set("status","patrol");
        	return 0;
        }
	message_vision(HIR "$N½ô½ôÁà×Å$nµÄÒÂÁì, $nÎŞ·¨ÍË³ö!\n" NOR, this_object(),me);
	return 1;
}

void die()
{
        object killer,room;
	if (objectp(killer=query_temp("last_damage_from")))
        {
        	room=find_object(query("startroom"));
        	killer->delete("being_caught");
        	killer->add("crime_lv",5);
/*      for (i=0;i<2;i++)
                {
                seteuid(getuid());
                ob=new("/d/choyin/npc/yamen_po.c");
                ob->set("call_out",1);
                ob->set("CatchWho",killer);
                ob->set("persuer",1);
                ob->set("startroom",query("startroom"));
                ob->move(environment());
                message_vision(HIR "´óµ¨¿ñÍ½£¬$N...¾ÓÈ»¸ÒÉ±Â¾¹«²î$n£¡Ğİ×ß£¬¿´µ¶£
¡\n" NOR,killer,this_object());
                ob->kill_ob(killer);
                if (stringp(file=find_command("kill")))
                        {
                        message_vision(file+"$N$n",killer,ob);
                        call_other(file,"main",killer,ob->query("id"));
                        }
                }
*/
        }
::die();
}

int accept_bribery(string arg)
{
	object money_ob,me,ob;
	string toid,money;
        int amount,bv1;
	me=this_player();
	if(!(me->query("being_caught"))) return 0;
	if (!arg||sscanf(arg,"%s with %d %s",toid,amount,money)!=3) return 0;
	if (!objectp(present(toid,environment())))
        	return notify_fail("ÄãÒª»ßÂ¸Ë­£¿\n");
	money_ob=present(money+"_money",this_player());
	if (!money_ob) return notify_fail("ÄãÉíÉÏÃ»ÓĞÕâÖÖ»õ±Ò¡£\n");
	if( amount < 1 ) return notify_fail("»õ±ÒÊ¹ÓÃÖÁÉÙÒªÒ»¸ö/Ã¿´Î¡£\n");
	if( (int)money_ob->query_amount() < amount )
                return notify_fail("ÄãÉíÉÏÃ»ÓĞÄÇ¶à" + money_ob->query("name") +"¡£\n");
	bv1 = money_ob->query("base_value");
	if( !bv1 ) return notify_fail("ÕâÖÖ¶«Î÷²»ÖµÇ®¡£\n");
	if (bv1*amount>=100000)
        {
	        write("ÕâÔõÃ´ºÃ?......ÒªÊÇÈÃÀÏÒ¯ÖªµÀÁË......\n");
	        write("Ê¢ÇéÄÑÈ´....ÎÒ¾ÍÊÕÏÂÁË..\n");
        	EMOTE_D->do_emote(this_object(),"grin",me->query("id"));
        	ob=new(base_name(money_ob));
        	ob->move(this_object());
        	ob->set_amount(amount);
        	money_ob->add_amount(-amount);
        	me->delete("being_caught");
        	if (me->query("crime_lv")>3)
                        me->add("crime_lv",-3);
        	else me->delete("crime_lv");
        	me->set_leader(0);
        	set("status","patrol");
        	set("CatchWho",0);
        	direction=GO;
        	return_home(find_object(query("startroom")));
        }
	else
        {
        	write("ÄãÕâÊÇÊ²Ã´ÒâË¼?......Ë­Ï¡º±ÄãµÄÇ®?!\n");
        	me->add("crime_lv",1);
        }
	return 1;
}

