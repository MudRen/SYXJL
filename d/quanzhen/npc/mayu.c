...
void create()
{
        ...
        set_skill("kongming-quan", 190);    //空明拳 
        set_skill("dodge",190);              // 基本躲闪
        set_skill("qixing-huanwei", 190);        //七星换位
        set_skill("parry", 200);             // 基本招架
        set_skill("sword", 200);             // 基本剑法
        set_skill("quanzhen-jianfa", 200);        //天罡剑法 
...
void init()
{
        ...
        ::init();
        ob = this_player();
        if((int)ob->query("shen")<=-100&&ob->query("family/master_id")=="master ma")
       {
