...
//		return notify_fail("媒婆说道：真是抱歉，请您等一下到红娘庄来找我吧。\n");
	...
	if( (string)me->query("class")=="bonze" ) 
  	...
  	if( me->query("unmarried") > 5 )
		return notify_fail("媒婆睁大了眼睛说道：结婚专业户？\n");
	...
	card->set("name","你和"+obj->query("name")+"的结婚证");
	card->move(me);
	card=clone_object("/d/city/obj/marry_card");
	card->set("name","你和"+me->query("name")+"的结婚证");
	card->move(obj);
	say("媒婆递给"+obj->query("name")+"和"+me->query("name")
		+"一人一张结婚证书.\n");
	...
	if ((string)obj->query("gender") == "女性") {
        ...
        message("shout", HIC "【鹿鼎记】：" + HIR + "今天" + me->query("name") + "和"
	+ obj->query("name") + "喜结连理！\n"+"\n"+
         "
	         ■        ■ 
	    ■■■■■■■■■■■
