...
今天想从这里出去就跟我"HIR"(bihua)"NOR+CYN"比划比划，如果赢了，我就闪开一条路，让你走，否则就爬回武馆去！\n"NOR, this_object(), ob);
}
        else {
        message_vision("$N谄媚地对$n说道：这位"+RANK_D->query_respect(ob)+"武功高强，实在是英雄出少年，您老请，您老请。\n"NOR, this_object(), ob);
                   ob->move("/d/village/hsroad1");
        }
}
int do_bihua()
{
