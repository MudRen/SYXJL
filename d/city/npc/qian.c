...
int do_check()
{
	...
	if (!total || total < 0)
	{
        ...
        write("钱眼开悄悄告诉你：您在弊商号共存有" + MONEY_D->money_str(total) + "。\n");
	return 1;
}
...
/*int do_kill(string arg)
{
	...
	if( ob->query("name")=="钱眼开" && !userp(ob))
	{
		command("chat 各位英雄救命啦。。。"+me->name()+"这个"+RANK_D->query_rude(me)+"来钱庄打劫 ！");
		kill_ob(me);
		me->kill_ob(ob);
		return 1;
	}
...
/*void greeting(object ob)
{
	if ( ( ob->query("combat_exp") * 100 < ob->query("balance") ) && ob->query("balance") > 10000 )
	{
		...
		{
			write("钱眼开对你说道：这位"+RANK_D->query_respect(ob)
				+"，因为前几天钱庄被打劫，还好，您的存款没有损失！\n");
			ob->save();
		}
		else
		{
