...
*/
	...
	if( userp(ob2) && (object)ob2->query_temp("pending/fight") !=ob1 ) {
		...
		tell_object(ob2, YEL "如果你愿意和对方进行比试，请你也对" + ob1->name() + "("+(string)ob1->query("id")+")"+ "下一次 bihua(fight)指令。\n" NOR);
...
void check(object ob1, object ob2)
{
	...
	command("chat " + ob1->query("name")
		+ "(" + ob1->query("id") + ")与" + ob2->query("name") + "(" + ob2->query("id") + ")，现在擂台上开始比武夺帅！\n");
...
int observe(object ob1,object ob2)
{
	...
	{
		command("chat " + ob1->query("name") + "(" + ob1->query("id") + ")被打落擂台了！\n");
//		command("chat " + ob1->query("id") + "被打落擂台了！\n");
	...
	{
		command("chat " + ob2->query("name") + "(" + ob2->query("id") + ")被踢下擂台了！\n");
...
//		command("chat " + ob2->query("id") + "不愿继续比武了！\n");
	...
	if ( ob1->query("qi")*2 > ob1->query("max_qi"))
	{
		if  (ob2->query("qi")*2 > ob2->query("max_qi")) {
			command("chat " + ob1->query("name")
				+ "(" + ob1->query("id") + ")与" + ob2->query("name") + "(" + ob2->query("id") + ")比武不分胜负！\n");
			return 1;
		}
		command("chat " + ob1->query("name")
			+ "(" + ob1->query("id") + ")比武战胜" + ob2->query("name") + "(" + ob2->query("id") + ")！\n");
	...
	else
	{
		command("chat " + ob2->query("name")
			+ "(" + ob2->query("id") + ")比武战胜" + ob1->query("name") + "(" + ob1->query("id") + ")！\n");
		ob = new("clone/money/silver");
		ob->set_amount(2);
		ob->move(ob2);
	}
