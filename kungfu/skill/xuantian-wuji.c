...
{
        int i = (int)me->query_skill("xuantian-wuji", 1);
        int t = 1, j;
        ...
        if ((int)me->query_skill("force", 1) < 10)
                return notify_fail("你的基本内功火候还不够，还不能学习玄天无极功。\n");

        if(i<=100)
        {
                if (i > 10 && (int)me->query("shen") < t * 100) 
                        return notify_fail("你的侠义正气太低了，无法领会更高深的玄天无极功。\n");
        }
        else
        {
        ...
        || me->query_skill("yunlong-shengong",1))
                return notify_fail("你不先散了别派内功，怎能学玄天无极功？！\n");
