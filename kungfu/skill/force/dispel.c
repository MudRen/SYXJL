...
int exert(object me, object target)
{
        ...
        {
                message_vision(HIW "\n$N深吸一口气，又缓缓的吐了出来。\n" NOR, me);
                tell_object(me, YEL "你默运" + force_name + "，开始排除身体中的异常症状。\n\n" NOR);
                 me->add("neili", -100);
        } else
        {
                message_vision(HIW "\n$N深吸一口气，将手掌粘到$n的背后。\n" NOR,
        ...
        if (conditions)
        {
                ...
                for (i = 0; i < sizeof(cnds); i++)
                {
                        ...
                        if (me == target)
                        {
                                tell_object(me, WHT "你调息完毕，将内力收回丹田。\n\n" NOR);
                        ...
                        } else
                        {
                                ...
                                message_vision(WHT "$N将手掌从$n背后收了回来。\n\n" NOR, me, target);
                        }
                        return 1;
                }
        }
        ...
        if (still_ill)
        {
                if (me == target)
                {
                        ...
                        message_vision(HIG "$N脸色变了变，有些不太自然。\n\n" NOR, me);
                        me->start_busy(3 + random(3));
                } else
                {
                        tell_object(me, MAG "你运功良久，没能发挥半点作用。\n" NOR);
                        tell_object(target, MAG "你觉得内息一阵紊乱，说不出的难受。\n"
                                            "看来" + me->name() + "是不能给予你帮助了。\n" NOR);
                        message_vision(HIG "$N将手掌从$n背后收了回来，脸色说不出的难看。\n\n" NOR,
                                       me, target);
        ...
        } else
        {
                if (me == target)
                {
                        ...
                        message_vision(WHT "$N眉角微微一动，随即恢复正常。\n\n" NOR, me);
                        me->start_busy(1);
                } else
                {
