...
string eff_status_msg(int ratio)
{
        ...
        if( ratio > 90 ) return HIG "似乎有些疲惫，但是仍然十分有活力。" NOR;
        if( ratio > 80 ) return HIY "看起来可能有些累了。" NOR;
        if( ratio > 75 ) return HIY "看起来可能受了点轻伤。" NOR;
        if( ratio > 70 ) return HIY "动作似乎开始有点不太灵光，但是仍然有条不紊。" NOR;
        if( ratio > 65 ) return HIY "受了几处伤，不过似乎并不碍事。" NOR;
        if( ratio > 60 ) return HIY "受伤不轻，看起来状况并不太好。" NOR;
        if( ratio > 50 ) return HIY "气喘嘘嘘，看起来状况并不太好。" NOR;
        if( ratio > 45 ) return HIR "气息粗重，动作开始散乱，看来所受的伤着实不轻。" NOR;
...
string damage_msg(int damage, string type)
{
        ...
        case "刺伤":
                ...
                return "结果只听见$n一声惨嚎，$w在$p身上对穿而出，鲜血溅得满地！！\n";                
                break;
        default:
                if( !type ) type = "伤害";
                str =  "结果造成非常可怕的严重";
