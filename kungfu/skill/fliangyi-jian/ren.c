...
int perform(object me, object target)
{
    ...
    if ((int)me->query_skill("fliangyi-jian", 1) < 60 )
        return notify_fail(WHT"你的反两仪剑法不够娴熟，使不出「天地同仁」。\n"NOR);
       ...
       random(me->query("combat_exp")) > (int)target->query("combat_exp")/3 )
    {
        ...
        damage = (int)me->query_skill("fliangyi-jian", 1) + (int)me->query_skill("sword", 1)/2;
