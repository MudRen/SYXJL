...
int perform(object me, object target)
{
        ...
        if( (int)me->query_skill("beiming-shengong", 1) < 40 )
                return notify_fail("你的北冥神功不够娴熟，不能种生死符。\n");
        ...
        if (!objectp(obj=present("jiudai", me)))
          ...
          msg = HIW "$N化水为冰，凝于掌中，轻飘飘地拍向$n。\n";

        me->start_busy(1);
        if( random( (int)me->query_skill("liuyang-zhang",1))
          > random(target->query_skill("dodge") ) ) {
                    msg += HIG " 结果$p被$P攻了个正着！$p中了$P的生死符了！\n" NOR;
                target->receive_damage("qi",(int)me->query_skill("beiming-shengong",1));
