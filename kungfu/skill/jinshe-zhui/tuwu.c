...
//                return notify_fail(target->name() + "已经被毒雾迷住了，你还吐什么雾？？想毒自己啊！！\n");   
        if( (int)me->query_skill("jinshe-zhui", 1) < 150 )
...
//                return notify_fail("你的金蛇剑法不够娴熟，不会使用金蛇吐雾。\n");
        ...
        if( random( (int)me->query_skill("jinshe-zhui",1))
        ...
        target->start_busy( (int)me->query_skill("jinshe-zhui",1) / 40 ) + random( (int)me->query_skill("jinshe-zhui",1) / 100 );
