...
int exert(object me, object target)
{
	...
	attackp = me->query_skill("huagong-dafa", 1) + me->query_skill("beiming-zhenqi", 1) + me->query_skill("zhaixinggong",1) / 3;
	defendp = target->query_skill("force") + target->query_skill("dodge");

	message_vision( HIW"$N逆运北冥真气，掌中凝出无数晶莹闪烁的冰晶，在寒光闪耀中，向$n袭来！\n\n"NOR, me, target );

	if( attackp > defendp / 2 + random(defendp) / 2 ) {
		message_vision( HIW"无数闪耀的冰晶围绕着$n的全身，渐渐凝聚成冰块，将$n冻在其中！\n"NOR, me, target );
//		seteuid(ROOT_UID);
		target->disable_player(HIW"<冻僵中>"NOR);
		target->set_temp("block_msg/all", 1);
                call_out("remove_frozen", 7 + random((int)me->query_skill("huagong-dafa", 1) / 50) + ((int)me->query_skill("beiming-zhenqi", 1) / 50), target);
	}
	else {
