...
//        if( !me->is_fighting() || !target->is_fighting())
	...
	message_vision(
        ...
        sp = me->query_skill("force") + me->query_skill("dodge") + me->query_skill("pmshenfa",1)/2;
...

	{
		...
		tell_object(me, HIG "你觉得" + target->name() + "的丹元自手掌源源不绝地流了进来。\n" NOR);
                ...
                target->add("max_jingli",  -1*(1+(me->query_skill("xixing-dafa", 1)-100)/5) );

                me->add("max_jingli",       1*(1+(me->query_skill("xixing-dafa", 1)-100)/5) );

                me->add("jingli",       1*(1+(me->query_skill("xixing-dafa", 1)-100)/5) );
