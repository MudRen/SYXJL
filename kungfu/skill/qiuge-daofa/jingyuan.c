...
HIG "翠辇不曾过－－$N手中$w似重逾千斤，磕向$n的$l！\n" NOR,
	...
	||	!me->is_fighting(target) )
		...
		return notify_fail(HIC "［景远声幽］只能对战斗中的对手使用。\n"NOR);
	...
	if(!obs[i]->query_skill("dienianhua-jian")) {FLAG=0; continue;}

	if(!objectp(ob=obs[i]->query_temp("weapon"))) {FLAG=0; continue;}

	if(ob->query("skill_type") != "sword"){FLAG=0; continue;}

	if(obs[i]->query_skill_mapped("sword") != "dienianhua-jian") {FLAG=0; continue;}
