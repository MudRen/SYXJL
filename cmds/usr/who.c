// From ES2
// Modified by zxp@cdfjnet Oct 6, 1997
// modify bye Ronger 01/99
 ...
 [-l] [-w] [-p] [<ID>]\n");
        ...
        else
        {
                ...
                while ( i-- )
                        ...
                        // Skip those users in login limbo.
                        if ( !environment(list[i]) ) continue;
                      if(me) if(objectp(me)) if ( !me->visible(list[i]) ) continue;
                        if ( interactive(list[i]) && query_idle(list[i]) > 150)
                                ...
                                name = sprintf( HIM"%s%-10s(%-11s"RED"|",
                                        " ", list[i]->name(1),
                                        capitalize(list[i]->query("id"))+")");
...
str += NOR+"────────────────────────"+HIG"SYXJL"NOR"─────────\n";
          str = sprintf("%s共有 "+HIY"%d"+NOR" 位使用者连线中，系统负担：%s\n "HIY"@"NOR" 表示发呆中，"HIB"#"NOR" 表示断线中。\n",
                str, ppl_cnt,
                query_load_average());
        if ( remote ) return str;
        if ( !arg || arg == "" ) write(str);
        if ( arg ) me->start_more(str);
        return 1;
}
