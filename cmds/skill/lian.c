...
int main(object me, string arg)
{
        ...
        sscanf(arg,"%s %d",arg,times);   
          ...
          if(times>30)
                return notify_fail("你也太贪了，最多做30次！\n");
        ...
        if (me->is_busy())
              ...
              if(check_valid_create(skillname))
                return notify_fail("你自己创建的功夫只能用selfpractice和selfthinking来提高。\n");
        if( me->is_fighting() )
        ...
        for(i=0;i<times;i++)
           {
               if( SKILL_D(skillname)->practice_skill(me) ) {
                me->improve_skill(skillname, skill_basic/5 +1, skill_basic > skill? 0: 1);
                  write( HIG "你开始专心致致的练习" + to_chinese(skillname) + "这项技能！\n" NOR);
                }
               else return 0;
           }
        return 1;  
