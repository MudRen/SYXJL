...
int main(object me, string arg)
{
	...
	msg += "-----------------------------------------------------------\n"CYN;
	msg += "等  级： "HIY+ skl[arg]+CYN; 
	msg +="		升级熟练度： %"HIY+100*(int)lrn[arg]/(int)((skl[arg]+1) * (skl[arg]+1))+"\n";
	...
	if(SKILL_D(arg)->valid_enable("force") )	 msg = CYN"特殊运用 (yun/exert) \n\n"HIW;
		else msg = CYN"特殊攻击 (perform) \n\n"HIW;
	...
	if (w == 0) msg += "「"+to_chinese(arg)+"」正在整理中。\n";
		...
		msg += "「"+to_chinese(arg)+"」目前共有"+chinese_number(w)+"种"+
