...
int respirating(object me)
{
        ...
        if (jingli_gain > respirate_cost) jingli_gain = respirate_cost;
        ...
        if ((int)me->query("jingli") < (int)me->query("eff_jingli") * 2)
                return 0;
        if ((int)me->query("max_jingli") > (int)me->query_skill("force", 1) * 8) {
                write("你的精力修为已经达到了你内功所能控制的极限。\n");
...
{
        if (me->query("jingli") > me->query("eff_jingli") * (2 + (me->query_skill("force") / 100)))
        me->set("jingli", me->query("eff_jingli") * (2 + (me->query_skill("force") / 100)));
        me->delete_temp("pending/respirate");
