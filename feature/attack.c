...
{       
        ...
        for(i=0;i<sizeof(inv);i++)
        {
                ...
                else if(inv[i]!=me && inv[i]->query_temp("baohuman")==ob->query("id") )
                {
                        tell_object(inv[i], HIR ""+ob->query("name")+"受到攻击，你挺身而出，加入战团！\n" NOR);
                        message_vision(HIY"$N挺身而出，加入战团!\n"NOR,me);
                        inv[i]->kill_ob(me);
                }
        }
        fight_ob(ob);
...
int hate(object me,object ob)
{
        ...
        int i,degree,score1,score2,mp_num; 
        ...
        if( degree>600 && score2>200 && !userp(ob) )
           {call_out("meet",0,"hi "+ob->query("id"),ob); 
            return 1; 
            }
...
{
        set_temp("competition", ob);
        ob->set_temp("competition", this_object());
        fight_ob(ob);
        ob->fight(this_object());
}
void win()
{
        delete_temp("competition");
}
