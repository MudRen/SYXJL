...
mapping mixed_poison(mapping p1, mapping p2)
{
        ...
        if (p1) r = (p1["remain"] ? p1["remain"] : p1["level"] * p1["duration"]);
        else    r = 0;

        if (p2) r += (p2["remain"] ? p2["remain"] : p2["level"] * p2["duration"]);
...
int update_condition(object me, mapping cnd)
{
	...
	if(! cnd || me->query_temp("nopoison")) return 0;
	...
	me->receive_wound("jing",  jd / 2);
        me->receive_damage("qi",   qd);
	me->receive_wound("qi",    qd / 2);

        if (cnd["id"] == "nature poison" ||
            cnd["level"] / 2 + random(cnd["level"]) < (int)me->query_skill("force"))
        {
