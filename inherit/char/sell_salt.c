...
int sell_salt(string arg)
{
    ...
    if (me->query("need_salt")>0){
        if (me->query("need_salt")<amount){
            command("shake");
            command("say 本店只需要"+CHINESE_D->chinese_number(me->query("need_salt"))+"斤盐。");
            return 1;
        }
    ...
    message_vision("$N把身上的" + CHINESE_D->chinese_number(amount)+ obj->query("base_unit")+obj->name() + "卖掉"+ string_value(0,value)+"。\n",
        this_player());
    me->add("need_salt",-amount);
        bonus = (int)this_player()->query("potential");
        bonus = bonus - (int)this_player()->query("learned_points");
        bonus = bonus + amount/2;
        if ( bonus > potential_lv((int)this_player()->query("combat_exp")) )
        bonus = potential_lv((int)this_player()->query("combat_exp"));
        bonus += (int)this_player()->query("learned_points");
        this_player()->set("potential", bonus );           
