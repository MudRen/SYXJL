...
mixed hit_ob(object me, object ob, int damage)
{
	...
	if (damage / 3 + random(damage * 2 / 3) <
            ...
            ob->query("neili") > damage / 5)
        {
                if (ob->query("qi") < 150)
                {
                        msg = MAG "你觉得伤口有些发麻，连忙运功化解，但是一时体力不支，难以施为。\n" NOR;
                } else
                if (ob->query("jing") < 60)
                {
                        msg = MAG "你觉得伤口有些发麻，连忙运功化解，但是一时精神不济，难以施为。\n" NOR;
                } else
                if (ob->query("neili") < damage / 5 + 50)
                {
                        msg = MAG "你觉得伤口有些发麻，连忙运功化解，但是一时内力不足，难以施为。\n" NOR;
                } else
                {
                        ob->add("neili", -damage / 5);
                        ob->receive_damage("qi", 20);
                        ob->receive_damage("jing", 10);
                        return MAG "你觉得被咬中的地方有些发麻，连忙运功化解毒性。\n" NOR;
                }
        }
