...
void inside_group(string line, object ob, int idx, int sidx)
{
    ...
    if(line == "q" )  {
       ...
       write("\n想留言吗？("+HIR"q"NOR+" 退出"+HIR " p 标题"NOR+" 留言)");
...
void do_readnewsgroup(object ob, int idx)
{
    ...
    if(!sizeof(news[idx]["index"]))  {
       write("这个新闻组目前没有任何留言。\n");
       write("你要留言吗？("+HIR"q"NOR+" 退出"+HIR "p 标题"NOR+" 留言)");
       ...
       write("\n");
       write("请选择要读的留言：(q 退出 l 重列  p 标题 贴文 d 删去 号码进入)");
...
int do_makegroup(string arg)
{
  ...
  string name, id;
   ...
   if(!arg||sscanf(arg, "%s %s", name, id) != 2)
...
int do_readgroup(string arg)
{
    ...
    write("\n"+HIR+"=================================================================="+NOR+"\n");
    write("欢迎使用剑.花.烟雨.江南 " + HIG + "BBS " + NOR + "板！\n");
    write(HIR+"=================================================================="+NOR+"\n");
    for(i=0; i<sizeof(news); i++)   {
...
int do_cpbboard(string arg)
{
    ...
    if(!arg||sscanf(arg, "%s %s", bb, newsgroup) != 2 )
     return notify_fail("指令格式：cpbboard b_board newsgroup\n");
   
    bb_file = "/clone/board/" + bb;        
