...
string long()
{
	...
	{
		j = sizeof(notes)-i-1;
		msg += sprintf("%s[%2d]" NOR "  %-40s %14s (%s)\n",
			( notes[j]["time"] > last_time_read ? HIY: ""),
			j+1, notes[j]["title"],
			notes[j]["author"] + "(" + notes[j]["postid"] + ")" ,
			ctime(notes[j]["time"])[0..15] );
	}
	return msg;
}
...
int do_post(string arg)
{
	...
	{
		note["title"] = arg;
		note["author"] = HIY+this_player()->query("name")+NOR;
		note["postid"] = HIY+this_player()->query("id")+NOR;
		note["time"] = time();
	}
	else
	{
		note["title"] = arg;
		note["author"] = GRN+this_player()->query("name")+NOR;
		note["postid"] = GRN+this_player()->query("id")+NOR;
		note["time"] = time();
	}		
...
"[%d] %-40s %s(%s)\n----------------------------------------------------------------------\n",
		num + 1, notes[num]["title"], notes[num]["author"] + "(" + notes[num]["postid"] + ")" , ctime(notes[num]["time"])[0..9])
		+ notes[num]["msg"] );
