...

{
	...
	{
		my["eff_douzhi"] = 100;
		if ((int)ob->query_skill("buddhism", 1)){
			my["eff_douzhi"] += (int)ob->query_skill("buddhism", 1)/4;
//                         my["eff_douzhi"] += my["kill"]/4500;}
                }
		if ((int)ob->query_skill("taoism", 1)){
			my["eff_douzhi"] += (int)ob->query_skill("taoism", 1)/4;
//                          my["eff_douzhi"] += my["kill"]/5500;}
	...
	if( userp(ob) || undefinedp(my["max_qi"]) ) {
		...
		// 道家练气：３０岁前补气，３０岁后长气
			...
			if((qi_age=(int)ob->query_skill("taoism", 1)) > 39 && ob->query_skill("taiji-shengong", 1))

			{

				my["max_qi"] = my["con"] * 7 + 100;
				...
				if (qi_age > 0)
					...
					my["max_qi"] += qi_age * ((int)ob->query_skill("taiji-shengong", 1) / 10);

			}

			else {

				my["max_qi"] = my["con"] * 11 + 100;

				qi_age = my["age"] * 11 / 9 + 3;

				if (qi_age > 30) qi_age = 30;

				qi_age += my["max_jingli"] / 50;

				my["max_qi"] += qi_age * ((int)ob->query_skill("force", 1) / 11);
