...
({
        ...
        HIY"第十二榜  环宇三飞星--------------------------  B12"NOR,
        HIY"第十三榜  八方三神鞭  ------------------------  B13"NOR,
        HIY"第十四榜  中原八大锤  ------------------------  B14"NOR,
        HIY"第十五榜  神州三大杖客  ----------------------  B15"NOR,
        HIY"第十六榜  武林三大棍王  ----------------------  B16"NOR,
});
string printdate = NATURE_D->game_time();

void create()
{
        set_name(HIY"随缘洗剑录"NOR, ({ "topten", "board" }));
        set_weight(800);
        set("unit", "本");
        set("long","一本厚厚的洗剑录,记载了多少爱,恨,情,愁。你可以用(READ)来查阅。\n");
        set("value", 1600);
        set("no_get", 1);
        set("no_steal", 1);
        set("no_put", 1);
...
void init()
{
        ...
        seteuid(getuid());
        if( ob = new("/adm/daemons/topten/topten") )
