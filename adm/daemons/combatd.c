...
varargs int do_attack(object me, object victim, object weapon, int attack_type, string action_p)
{
        ...
        mixed foo;
        int ap, dp, pp;
        int damage, damage_bonus, defense_factor,temp=0;
// ±ÈÎäÏÖ³¡×ª²¥ start
        player=users();
// ±ÈÎäÏÖ³¡×ª²¥ end
	...
	if( dp < 1 ) dp = 1;
		...
		pp = pp * my["eff_douzhi"] / (temp + my["eff_douzhi"]);
                ...
                if( random(100)< pp*100/(ap+pp) ) 
                {
                        ...
                        //Ôö¼Ó·ÀÊØĞÅÏ¢
                        parry_msg = SKILL_D(parry_skill)->query_parry_msg(weapon);

                        if (! stringp(parry_msg))
                                parry_msg = SKILL_D("parry")->query_parry_msg(weapon);
