// securityd.c by aeolus 12/25/2000 圣诞节 xixi :)
#include <login.h>
...
// 对文件的写权要经过两级判断：
// 1.trusted_write中设置的euid(英文id)或status(巫师等级)具有写权；
...
// 2.exclude_write中设置的euid(英文id)或status(巫师等级) 不 具有写权；
//   exclude_write比trusted_write具有优先级，即
//   在exclude_write中设置的权限如果在trusted_write中同样设置仍然 不 具有写权；
//   exclude_write的设置方法可参看exclude_read。
...
// exclude_read的权限设置参看exclude_write的注释。
