...
int process_list(string idx, mapping info)
{
	...
	while (j--) if (sscanf(inf[j], "%s:%s", name, value) == 2)
		neww[name] = value;
	if(!neww["NAME"]) return 0;

	// make sure the name is in the proper form
	name = htonn( neww["NAME"] );
	while( name[strlen(name)-1] == '.' )
		name = name[ 0..strlen(name)-2 ];
	neww["ALIAS"] = nntoh(neww["NAME"]);

	// already know about ourselves
	if (neww["NAME"] == Mud_name()) return 0;
	...
	if (!old) DNS_MASTER->set_mud_info(name, neww);
	else {
		inf = keys(neww);
		j = sizeof(inf);
		while (j--) old[inf[j]] = neww[inf[j]];
