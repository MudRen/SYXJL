/*
 * File    : remote_a.c
 * Creator : ken@XAJH
 * When    : 98-1-15
 *
 * This protocol is used to receive a intermud emote request.
 */

#define DEBUG "debug"


#include <net/dns.h>
#include <net/macros.h>

inherit F_CLEAN_UP;
inherit F_DBASE;

void create() {
	seteuid(ROOT_UID);
	set("channel_id", "remote_a");
}

void incoming_request(mapping info)
{
	object ob;
	string msg;
	mapping minfo;
	int code;

	if( !ACCESS_CHECK(previous_object())) return;

	minfo = (mapping)DNS_MASTER->query_mud_info(info["NAME"]);
	if (!minfo) return ;

	code = LOCATE_A->encoding(minfo["HOSTADDRESS"]);
	if( minfo["ENCODING"] == "GB" )	code = 0;

	if (stringp(info["PORTUDP"]) && stringp(info["HOSTADDRESS"])) {
		if (info["NAME"] == Mud_name())	return ;
		if (!DNS_MASTER->dns_mudp(info["NAME"]))
			PING_Q->send_ping_q(info["HOSTADDRESS"], info["PORTUDP"]);
		ob = find_player(lower_case(info["SOURCE_ID"]));
		if (!ob) return;

		if( code == 1 )
			msg = EMOTE_D->do_emote(ob,lower_case(info["MSG"]),LANGUAGE_D->toGB(info["TARGET"]),4);
		else
			msg = EMOTE_D->do_emote(ob,lower_case(info["MSG"]),info["TARGET"],4);
#ifdef DEBUG
	CHANNEL_D->do_channel(this_object(), DEBUG, 
		sprintf("receive message [%s %s].\n",info["MSG"],info["TARGET"])
	);
#endif

		if (!stringp(msg)) {
			CHANNEL_D->do_channel(ob,"sys","emote daemon return null.");
			return;
			}

		if(!userp(ob)) return;
		CHANNEL_D->do_channel(ob, info["CHANNEL"],msg,1,1);

	}
}
