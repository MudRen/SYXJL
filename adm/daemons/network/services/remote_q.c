 ...
 * When    : 98-1-15
 *
 * This protocol is used to process intermud emote request.
 */
...
void incoming_request(mapping info)
{
	...
	if (info["NAME"]) {
		...
		code = LOCATE_A->encoding(rhost["HOSTADDRESS"]);
		if( rhost["ENCODING"] == "GB" )	code = 0;
		...
		if( code )
			...
			"||MSG:"  + info["MSG"] +
			"||TARGET:" + LANGUAGE_D->toBig5(sprintf("mud=%s name=%s id=%s age=%d gender=%s respect=%s rude=%s ",
