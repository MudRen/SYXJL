...
string damage_msg(int damage, string type)
{
        ...
        case "刺伤":
                ...
                else return "结果只听见$n一声惨嚎，$w已从$p的$l对穿而出，鲜血溅得满地！！\n";
                break;
        case "瘀伤":
...
// 比武现场转播 end
        ...
        if( (string)me->query("env/combat") ) {
                if( damage > 0 )
                        tell_object(me, sprintf( HIW "AP：%d，DP：%d，PP：%d，伤害力：%d\n" NOR,
        ...
        if( (string)victim->query("env/combat")) {
                if( damage > 0 )
                        tell_object(victim, sprintf( HIW "AP：%d，DP：%d，PP：%d，伤害力：%d\n" NOR,
                                ap/100, dp/100, pp/100, damage));
                else
...
// 比武现场转播 start
                        ...
                        for (userno=0; userno<sizeof(player); userno++)
                                if (player[userno]->query_temp("view_leitai"))
                                        tell_object(player[userno], "\n"+ob->name()+"扑在地上挣扎了几下，腿一蹬，口中喷出几口"HIR"鲜血"NOR"就死了。\n");
// 比武现场转播 end
                        break;
                case "unconcious":
                        message_vision("\n$N脚下一个不稳，跌在地上一动也不动了。\n\n", ob);
...
sprintf("%s将官府通缉犯%s就地正法，获得了扬州衙门的特殊奖励!",killer->name(),victim->name()));
...
// by rhxlwd
                ...
                CHANNEL_D->do_channel(this_object(), "rumor",
                        sprintf("据说%s"+msg+"真是惨不忍睹啊。", victim->name(1)));
                if ( userp(killer) && !killer->query_temp("kill_killer")) {
                                CHANNEL_D->do_channel(this_object(), "rumor",
                                sprintf("%s被官府通缉了。", killer->name(0)));
                        }
