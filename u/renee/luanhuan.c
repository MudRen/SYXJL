...
int zhen(object me, object target)
{
	...
	if ( random(me->query_skill("taiji-quan", 1)) > 199 )
	{
		type = "瘀伤";
	...
	else if (random((me->query_skill("taiji-quan") + me->query_skill("force")) * me->query_int() / 100) >
		(target->query_skill("parry") + target->query_skill("dodge")) * target->query_int() / 200 )
	{
		...
		if(type == "瘀伤")
			damage = damage+me->query_skill("taiji-quan")/2;
