...
{
	write("\n"HIG"\n\n¡¼ĞÂÂ¹¶¦¼Ç¡½"NOR"»¶Ó­ÄúÀ´·Ã£¡Ê¹ÓÃ¹ú±êÂëµÄÍæ¼ÒÇë¼üÈë£º¡¼GB¡½\n");
...
//		write("                       Use GB encoded Chinese.\n\n");
		write("\n»¶Ó­Äú½øÈë"HIG"¡¼ĞÂÂ¹¶¦¼Ç¡½"NOR"µÄ´«ÆæÊÀ½ç¡£\n\n");
	else
...
//			if( !wizardp(me) || (wizardp(me) && me->query("env/debug")) )
	...
	output += "©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤\n";
	output += sprintf("¹²ÓĞ %d ¸ö¡¶Â¹¶¦¼Ç¡·ÏµÁĞÕ¾µãÓë±¾Õ¾Á¬ÏßÖĞ¡£¹²ÓĞ %d ¸öÍæ¼ÒÔÚÏß¡£\n\n",total==0?0:total-1,p_count);
...
void login(string arg, object ob)
{
	...
	welcome_msg1 = "/adm/etc/welcome1";	
	welcome_msg2 = "/adm/etc/welcome2";		
        switch( random(3) ) 
        {
                case 0:
    ...
    printf(" "HIC"¡¾"HIM"ĞÂÂ¹¶¦¼Ç"HIC"¡¿"NOR"Ä¿Ç°¹²±£´æ×Å"YEL"%s"NOR"¸öÍæ¼ÒµÄµµ°¸¡£ \n"NOR,chinese_number(ppl1_cnt) );
    printf(" "HIC"¡¾"HIM"ĞÂÂ¹¶¦¼Ç"HIC"¡¿"NOR"×Ô¶şÁãÁãÁãÄêÊ®¶şÔÂÒ»ÈÕÆğ£¬¹²ÓĞ"+YEL"%s"+NOR"ÈË´ÎÀ´·Ã¡£\n"NOR,chinese_number(login_num-1) );
printf(" "HIC"¡¾"HIM"ĞÂÂ¹¶¦¼Ç"HIC"¡¿"NOR"Ö÷Ò³µØÖ·: "+YEL"http://Mud.jztele.Com"+NOR"¡£\n" );
...
void get_passwd(string pass, object ob)
{
	...
	if( objectp(user = make_body(ob)) ) {
		if( user->restore() ) {
			...
			{
			write("ÄãÏÖÔÚÃ»ÓĞ½øÈëĞÂÂ¹¶¦¼ÇµÄÈ¨Àû£¬ÇëÒ»Ğ¡Ê±ºóÔÙÁªÏß¡£\n");
			destruct(user);
			destruct(ob);
			return;
		}
...
Ò»¸öÈËÎïµÄÌì¸³¶ÔÓÚËû»òËıËùĞŞÏ°µÄÎäÒÕÏ¢Ï¢Ïà¹Ø¡£ĞÂÂ¹¶¦¼ÇÖĞµÄÈËÎï´ó¶à¾ßÓĞ
ÒÔÏÂËÄÏîÌì¸³£º
...
/*	if (!user->query("opend/ĞÂÄêºØËê¿¨")) {
	...
	write("ĞÂÄê¿ìÀÖ£¡¹§Ï²ÄãµÃµ½ÁËÂ¹¶¦¼ÇµÄĞÂÄêºØËê¿¨£¡\n");
	}
*/
