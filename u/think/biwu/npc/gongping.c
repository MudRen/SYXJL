...
int do_start(string arg)
{
	...
	if(arg=="xunhuan") {
		...
		if(sizeof(inv)<4)
			return notify_fail("报名的人太少了，比武大会无法开始。\n");
                message("channel:chat",HIW"【比武大擂台】第"+CHINESE_D->chinese_number(i)+
	...
	for(a=0; a<sizeof(str); a++) {
		if(arg==str[a]) {
			...
			if(sizeof(menpai)<4)
				return notify_fail("报名的人太少了，比武大会无法开始。\n");
			message("channel:chat",HIW"【比武大擂台】"HIM+arg+HIW"掌门大弟子选拔"+
...
int xuanbu()
{
	...
	message("channel:chat",HIW"【比武大擂台】公平子朗声宣布道：第"+
...
int start_biwu()
{
	...
	if(!ob1 && !ob2) {
		message("channel:chat",HIW"【比武大擂台】公平子宣布道：第"+
...
int end_biwu()
{
	...
	if(sizeof(id)==1) {
		message("channel:chat",HIW"【比武大擂台】公平子朗声宣布道：由于第"+
	...
	message("channel:chat",HIW"【比武大擂台】公平子朗声宣布道：现在到了第"+
		CHINESE_D->chinese_number(i)+"届比武大会决赛的时候了：\n",users());
	message("channel:chat",HIW"【比武大擂台】公平子宣布道，决赛由：\n"NOR+YEL+
		"                "BLINK+
		ob1->query("name")+"("+id1+")"+"---V.S.---"+
		ob2->query("name")+"("+id2+")\n"NOR,users());
	ob->set("start/biwu_fighter1", id1);
...
int jieshu_biwu(string id1, string id2)
{
	...
	if(str=="xunhuan") {
		...
		if(!ob2) {
			...
			message("channel:chat",HIW"【比武大擂台】公平子朗声宣布道："+
				str+"掌门大弟子选拔比武到此结束！\n",users());
			message("channel:chat",HIR"        祝贺"+
				BLINK+ob1->query("name")+"("+id1+")"+
				"荣任"+str+"掌门大弟子！\n"NOR,users());
	...
	case "geren":
		ob->set("fangshi", BIWUA);
                message("channel:chat",
                        HIW"【比武大擂台】现在比武场开放使用，大家可以随意前来比试！\n"NOR,users());
		save();
		return 1;
	case "leitai":
		ob->set("fangshi", BIWUB);
                message("channel:chat",
                        HIW"【比武大擂台】现在比武场开放摆擂，欢迎各位高手前来摆擂！\n"NOR,users());
		save();
		return 1;
	case "menpai":
		ob->set("fangshi", BIWUC);
                message("channel:chat",
                        HIW"【比武大擂台】现在比武场开放各门派比武，请向公平子申请！\n"NOR,users());
		save();
		return 1;
	case "xunhuan":
		ob->set("fangshi", BIWUD);
                message("channel:chat",
                        HIW"【比武大擂台】现在第"+CHINESE_D->chinese_number(i)+
...
int do_agree(string arg)
{
	...
	message("channel:chat",
		HIW"【比武大擂台】巫师批准了"+family+"举办掌门大弟子选拔比武申请，"+
		"请从速前来报名参加！\n"NOR,users());
	save();
	return 1;
}
